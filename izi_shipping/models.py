...
class AvailableCompanyManager(ShippingCompanyManager):
    ...
    def for_address(self, addr):
        ...
        :param addr: izi.apps.address.models.UserAddress or subclassed instance (object must have 'line4' attr)
...
class ShippingCompany(AbstractWeightBased):
    ...
    LIST_SEPARATOR = getattr(settings, 'IZI_SHIPPING_LIST_SEPARATOR', ';')
    SHOW_IF_NOT_FOUND = getattr(settings, 'IZI_SHIPPING_IF_NOT_FOUND', True)
...
class ShippingContainer(models.Model):
    ...
    image = models.ImageField(
        _("Image"), upload_to=settings.IZI_IMAGE_FOLDER, max_length=255, blank=True)
